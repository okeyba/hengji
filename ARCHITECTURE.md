...
## 月度对账（勾对式 statement reconciliation，2026-06 评审通过）

**原则**：发现差额**不直接塞调整**，而是逐笔核对找出漏记/多记/记错并改正（补录/编辑/删除），调整降级为"查不到差错"的逃生口。GnuCash/Actual/YNAB 同路。

- **设置**：对账周期（每月最后一天/自定义）+ 提前提醒天数。本地 App 在打开时按「对账日−提前天数」判断弹横幅（OS 推送后置）。
- **流程**：选账户→填对账单余额（真实数）→该账户流水**逐笔带勾选**对账→实时算「已勾选合计」与「差额=对账单余额−已勾选合计」→差额≠0 则补录/改金额/删除把它对到 0→差额=0 才能「完成对账」。
- **数据**：`Posting` 加「已核销/已对账」标记 + `Reconciliation` 会话（账户+对账单余额+日期+完成时间）。补录/编辑/删除全复用现成 CRUD。**改金额＝重开整笔两腿按新额重展开**（保持平衡）；已核销分录被改动时弹提醒。
- **滚动状态**：账户全核完 → 账本本期「已对账」。多币种账户按原币对。
- **盘盈盘亏调整**（逃生口）：复用现有 `adjustBalanceEntry`，差额记入专门的「盘盈盘亏」科目。
