...
- **订单五态**：待采购/待发货/已发货/已完成/已取消。**财务事件只发生在「已完成」**（确认收入+出库+成本结转一次性入账）与取消；其余状态纯跟踪。初始态按库存充足度推导（任一商品在手不足→待采购）；**每个缺货商品各生成一张草稿采购单**（采购拆单 B，2026-06-13）——不同商品可来自不同供应商、各自独立确认，订单的草稿全部确认/作废后才转待发货。
...
### 账户全局化（2026-06-13 用户确认并落地，✅ 全 4 阶段）

**痛点**：小老板生意账与生活账常混在同一个真实账户（一个支付宝既付货款又付生活费）→ 报税/对账困难。
**模型**：账户带作用域 `Account.global?`——**真金白银账户（现金/银行/支付宝/微信）可标"全局共享"，所有账本共用**；**虚拟经营账户（应收/应付/库存商品/代采在途）+ 损益科目恒账本专属**。这是对「所有账户挂 bookId」的精化：全局账户的 `bookId` 仅作 home 元数据，余额归全局资金池、不计入某账本净资产。
- **分录铁律放宽**：原"一笔交易所有分录同账本"→"每条分录账户 = 该交易账本 **或** 全局账户"。于是生意账本可 `借支付宝(全局)/贷营业收入(生意)`。账本专属账户仍禁跨账本（守不可跨账本铁律）。
- **净资产重定义**：钱混在共享账户里 → 资金的资产负债表变**全局**。财务总表「全部净资产 = 全局资金(按 `global` 单列一次) + Σ各账本专属净额」，无重复计；每账本 Dashboard 净资产卡 → 「本月损益 + 经营资产(应收/库存−应付) + 可用资金(全局共享、只读引用)」。账本切分**损益与经营**，不切分共享资金。
- **对账按账户（不按账本）**：对账移侧栏全局入口；选账户跨账本勾对其全部流水；每条流水标所属账本 + 可按账本筛选核对各自小计；补录手动选账本（全局账户可选任意账本、专属账户固定其账本）。
- **落地**：4 阶段 P1 地基(`Account.global?` + store M14 `accounts.global` 列 + listAccounts"global OR 本账本" + 分录校验/对账放宽，既有账户全 global=false 行为不变) / P2 账户页「共享」开关 / P3 净资产报表重定义(AppData 加 `allTxns` 供全局账户跨账本聚合余额) / P4 对账全局化。详见 followups「⭐⭐ 账户全局化重构」。
...
- **Step 4 · 额外费用 + 公式引擎（flagship 核心）✅ 已落地（2026-06-13）**：账本级 `FeeDefinition`（声明式阶梯 `tiers`[{threshold,value}] + `calcType` percent/fixed/perQty），开单时商品行勾选应用（`OrderLine.feeIds`）。core 纯函数 `computeFees` 两遍：按「应用该费用的行」分组合计定档（阶梯取 ≤合计 的最高档、整组按该档率）→ 逐行/一次性算（percent:Σ行额×%/perQty:Σ行量×单位额/fixed:一次性）。**费用都当收入（B4）**——订单总额=商品+Σ费用、确认收入与应收按 gross、毛利按 gross。store M15 `fee_definitions` 表 + `order_lines.fee_ids`。web「费用」tab（阶梯档位编辑器）+ 开单每行勾选 + 实时预览（含跳档）。**简化**：费用统一进营业收入（不按费用名拆子科目）；非 CNY 订单 fixed/perQty 值按订单币种面值。**LLM 输入层后置（B6）**：FeeDefinition 即 LLM 自然语言翻译的目标格式，算账由 `computeFees` 确定性执行（红线：LLM 只翻译、不算账）。这是插件地基的第一块原语。
